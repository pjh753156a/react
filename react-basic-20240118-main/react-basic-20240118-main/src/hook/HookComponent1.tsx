...
// - useEffect : 컴포넌트가 렌더링될 때마다 특정 작업을 수행할 수 있도록 하는 함수, 
// mount, update, unmount 단계에서 수행되는 함수
// - useRef : DOM 요소에 직접 접근하기 위한 함수, 렌더링 사이에 값을 기억하는 함수
// - useContext : 컴포넌트 트리 상에서 전역적으로 상태를 공유하고자 할때 사용하는 함수
// - useReducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 
// 외부로 옮길 수 있게하여 코드관리를 조금더 수월하게 해줌
// - useCallback : 메모리 내에서 함수를 재사용할 수 있도록 하는 함수, 
// 렌더링 없이 특정 함수를 다시 호출할 수 있도록 해줌
// - useMemo : 메모리에 계산된 값(함수의 결과)을 저장하여 함수 재실행 없이 
// 사용할 수 있도록 하는 함수
...
export default function HookComponent1() 
{
    ...
    // 스코프할 상태 배열에 두 개 이상의 상태를 지정할 수 있음 (배열에 포함된 상태 중
    // 하나라도 변경되면 effect가 실행됨)
    useEffect(() => 
    {
