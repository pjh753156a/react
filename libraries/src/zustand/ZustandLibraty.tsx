...
// - Redux, MobX 와 같은 타 상태관리 라이브러리들에 비해 가벼움 - 빌드 할 때 패키징 속도 향상
// 빌드 후 파일의 크기가 작아짐
...
// - typescript에서 zustand의 create 함수를 사용할 때는 create 함수의 제너릭으로 store 타입을 
// 지정해야함
...
const useStore = create<store>((set) => ({ // 중괄호 안에 있는 store 객체
        zNormal : 0,
...
export default function ZustandLibraty() 
{
    ...
    // zustand로 선언한 상태 사용
    // const{상태들,상태변경함수들} = useStoreZustand훅 함수()
    ...
    <div>
        ...
        <div>
            <h4>zustand로 방식 : {zNormal}</h4>
            <button onClick={decreaseZNormal}>-</button>
            <button onClick={increaseZNormal}>+</button>
        </div>
        <SubComponent1 normal2={normal} increaseNormal2={increaseNormal} decreaseNormal2={decreaseNormal} />
        <SubComponent2/>
    </div>
...
interface Sub1Props {
    normal2 : number;
    increaseNormal2: () => void;
    decreaseNormal2: () => void;
}


function SubComponent1 ({normal2,increaseNormal2,decreaseNormal2}:Sub1Props)
{
    return(
        <div>
            <h5>Normal : {normal2}</h5>
            <button onClick={decreaseNormal2}>-</button>
            <button onClick={increaseNormal2}>+</button>
        </div>
    )
}
