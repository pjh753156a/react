...
{  
    ...
    axios
    .get(GET_URL)
    // 각각의 http method 함수 결과를 then으로 받을 땐 response 객체를
    // 매개변수로 받는 콜백함수로 전달
    .then(response => {
        // response 객체는 status, header, data(body)를 포함하고 있음
        // then작업은 status code가 400,500번대가 아닌 response를 받아옴
        console.log(response.data);
    })
    .catch(error => {
        // 해당 request에 대해서 requset가 실패하거나 status code가 400,500번대이면
        // catch로 AxiosError 객체를 받아옴
        // response가 존재하는 에러는 error 객체에 response 포함되어 있음
        console.log(error);
    })
    ...
    axios.get(GET_URL,{
         headers:{'Authorizatidon':'Bearer 132123132132'},
         params:{'time':'1144'}
    })
    .then(response => {
        console.log(response.data);
    ...
    axios.patch(PATCH_URL,{email:'email@email.com',nickname:'그만하자이제응??'})
