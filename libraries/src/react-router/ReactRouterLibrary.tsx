...
// - root 경로에 있는 index.tsx의 root.render()안에 포함되어 있는 <App /> 컴포넌트를 
// <BrowserRouter> 컴포넌트로 묶어야함
// <BrowserRouter> : URL을 사용하여 브라우저 주소 표시줄에 현재 위치를 저장하고 탐색할 수 있도록
// 하는 컴포넌트 

// 특정 url 패턴에 대해서 컴포넌트를 각각 다르게 랜더링하고자 한다면 App.tsx에 <Routes> 컴포넌트와
// <Route> 컴포넌트를 이용하여 경로에 따른 컴포넌트를 지정
...
export default function ReactRouterLibrary() 
{
  ...
  // - 페이지가 새로 고쳐지면 상태 유지가 안됨
 
  // <Link>:
  // - 사용자가 해당 웹 애플리케이션 내에서 다른 페이지로 이동할 수 있도록 
  // 하는 컴포넌트
  // - a요소의 href로 페이지 전환을 하게되면 새로운 페이지로 전환되는 반면
  // <Link>는 새로고침을 하지 않고 URL만 변경 
  // - 다른 웹 애플리케션으로 이동하는 작업은 <Link>로 수행하지 않음
  // - 해당 <Link> 컴포넌트를 클릭했을 때 다른 작업을 같이 수행하고자 한다면
  // 그 작업이 정상적으로 수행되지 않음
  // - 페이지 이동 전에 특정 작업 결과에 따라 이동하고 싶으면 
  //  <Link> 컴포넌트를 사용하지 않음
  const onLinkClick = () => {
    alert('클릭!');
  }
  ...
  // - Navigator 함수는 특정 조건에 따라서 URL을 변경하고자 할 때 
  // 사용할 수 있음
